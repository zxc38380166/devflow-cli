...
✅ 本地分支已建立：feat/abc123-atm-reconciliation
✅ 分支已推送至 remote
```
...
const response = await fetch('https://api.trello.com/1/cards', {
  ...
  body: JSON.stringify({
    ...
    idLabels: ['label_id'],               // ← 根據專案自動帶入 FE/BE
...
| **develop 不凍結** | release 切出後，develop 繼續收下一版的 feat |
...
- Trello 卡片用 **Label** 區分 `FE` / `BE` / `fullstack`
...
Trello Card: "新增 ATM 對帳功能" [fullstack]
  │
  ├── BE repo: feat/abc123-atm-reconciliation-api
  │                 → PR 回 develop
  │
  └── FE repo: feat/abc123-atm-reconciliation-ui
                     → PR 回 develop
```
...
```bash
# 在前端 repo 內執行，自動偵測角色為 FE
cd my-app-web && devflow task

# 在後端 repo 內執行，自動偵測角色為 BE
